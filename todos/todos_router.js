...
router.delete("/:id", restrictedItem, async (req, res) => {
  ...
  try {
    ...
    if (deletedItem === 0)
      res.status(401).json({ message: "could not delete item - user error" });
    else
      res.status(201).json({
...
router.put("/:id", restrictedItem, async (req, res) => {
  ...
  try {
    ...
    if (changedItem === 0) res.status(401).json({ message: "no changes made" });
    else res.status(201).json(changedItem);
  } catch (err) {
    res.status(501).json({
...
function verifyNewItem(req, res, next) {
  ...
  else res.status(401).json({ message: "title and user_id fields required" });
}
